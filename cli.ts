...
export const enumToCli = <E>(e: Enum<E>) => {
  ...
  features.forEach((name) => {
    if (!good.test(name)) {
      throw new Error(
        `internal error: feature names must not contain weird fucking characters. yet here we have: "${name}"   (good names match this RegExp ${good.source})`,
      );
    }
    ...
    if (menu.map((x) => x.regularizedName).includes(regularizedName)) {
      const colliding = menu.filter((x) =>
        x.regularizedName === regularizedName
      )[0];
      throw new Error(
        `internal error: regularized feature names must be unique, yet here we are: ${name} regularizes to ${regularizedName} which is already in the menu (collides with ${colliding.name})`,
      );
    }
