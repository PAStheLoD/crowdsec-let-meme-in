...
const updateAndReload = () => {
    ...
    console.log(
        new Deno.Command("/bin/systemctl", { args: ["reload", "crowdsec"] })
            .outputSync(),
    );
};
...
const map = async (
    ...
    f: (x: string) => Promise<Date | null | false>,
) => Object.fromEntries(
    await Promise.all(
...
if (!lastModified.target) {
    console.log(
        "target file does not exist, creating it and reloading crowdsec",
    );
    updateAndReload();
}
...
if (lastModified.source > lastModified.target) {
    ...
    console.log(
        `no, source is older than target (${lastModified.source} > ${lastModified.target})`,
    );
}
